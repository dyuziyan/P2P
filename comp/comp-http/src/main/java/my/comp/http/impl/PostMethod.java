...
public abstract class PostMethod<R, P> implements HttpMethod<R, P> {
	...
	public R invoke(String url, P params) throws HttpException {
		...
		HttpPost httpPost = doGetHttpPost(url, params);
	...
	protected HttpPost doGetHttpPost(String url, P params) throws HttpException {
		...
		httpPost.setConfig(RequestConfig.custom().setConnectTimeout(TIMEOUT).setConnectionRequestTimeout(TIMEOUT).setSocketTimeout(TIMEOUT).build());// 设置2分钟50秒连接超时时间
		doSetParameter(httpPost, params);
	...
	protected abstract void doSetParameter(HttpPost httpPost, P params) throws HttpException;
